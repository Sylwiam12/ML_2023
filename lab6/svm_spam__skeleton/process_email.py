...
def process_email(email_contents: str) -> List[int]:
    ...
    inverted_dictionary={v: k for k, v in vocabulary_dict.items()}
    ...
    email_contents = re.sub('<(http:// | https://)[a-zA-Z]>', 'httpaddr', email_contents)

    # FIXME: Handle email addresses.
    # Convert all strings with @ in the middle to a 'emailaddr' token.
    email_contents = re.sub('<\B@+>', 'emailaddr', email_contents)

    # FIXME: Handle $ sign
    # Convert all sequences of $ signs to a 'dollar' token.
    email_contents = re.sub('<\A$>', 'dollar', email_contents)
    ...
    for token in tokens:
        ...
        #
        try:
            word_indices.append(inverted_dictionary[token])
        except:
            continue
